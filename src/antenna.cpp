...
void matrixMultiplication(const MatrixXf &A, const MatrixXf &B,
                          MatrixXf &result) {
  result = A * B;
}
 ...
 */
inline Matrix3f compute_rotation_matrix(const float azimuth,
                                        const float elevation) {
  Matrix3f rotation_matrix_yaw;
  rotation_matrix_yaw << cos(azimuth), 0.f, sin(azimuth), 0.f, 1.f, 0.f,
      -sin(azimuth), 0.f, cos(azimuth);

  Matrix3f rotation_matrix_pitch;
  rotation_matrix_pitch << 1.f, 0.f, 0.f, 0.f, cos(elevation), -sin(elevation),
 ...
 */
MatrixXf create_antenna(const Vector3f &position, const int columns,
 ...
 */
vector<int> used_sensors(const MatrixXf &antenna,
  ...
  for (int i = 0; i < normalized.rows(); i++) {
    ...
    if ((((x % SKIP_N_MICS) == 0) && ((y % SKIP_N_MICS) == 0)) ||
        (SKIP_N_MICS <= 0)) {
      used.push_back(i);
    }
  }
...
/**
 * Steer the antenna by creating a steered copy since this operation is
 * irreversibly due to noise
 * TODO fix negative signs for angles in order to steer array correctly
 */
MatrixXf steer(const MatrixXf &antenna, const float azimuth,
               const float elevation) {
  MatrixXf steered = antenna;

  steered *=
      compute_rotation_matrix(to_radians(-azimuth), to_radians(-elevation))
          .transpose();
...
MatrixXf compute_delays_lookup(const MatrixXf &antenna,
  ...
  VectorXf azimuth_angles = VectorXf::LinSpaced(
      azimuth_resolution, -azimuth_max_angle, azimuth_max_angle);
  VectorXf elevation_angles = VectorXf::LinSpaced(
      elevation_resolution, -elevation_max_angle, elevation_max_angle);
...
// MatrixXf generate_points_half_sphere(int n)
// {
//     double theta, phi;

//     double golden_ratio = (1 + pow(5, 0.5))/2;
//     for (int i = 0; i < n; i++)
//     {
//         theta = 2.0 * M_PI * ((double)i + 0.5) / golden_ratio;
//         phi = arccos(1.0 - 2.0 * ((double)i + 0.5) / (double)n);
//         if
//     }
// }
...
//      Matrix3f combined_rotation_matrix =
//      compute_rotation_matrix(to_radians(45.0), to_radians(0.0));
...
//     MatrixXf delays = compute_delays_lookup(antenna, azimuth_resolution,
//     elevation_resolution, AZIMUTH_MAX_ANGLE, ELEVATION_MAX_ANGLE); std::cout
//     << delays << std::endl;
...
//     std::cout << "Rotation matrix:" << std::endl << matrix2 <<
//     std::endl;//combined_rotation_matrix << std::endl;
//
...
//     //std::cout << "Vector3f" << matrix2.rowwise() + mid.transpose() <<
//     std::endl;
//     // std::cout << "middle " << matrix2.colwise() * middle.transpose() <<
//     std::endl;
