...
/** Fast memcpy write to a ringbuffer, requires N_SAMPLES % 2 AND BUFFER_LENGTH
 * % 2
 */
inline void write_buffer(ring_buffer *rb, float *in) {
...
//     memcpy(out, (void *)(data_ptr + rb->index), sizeof(float) *
//     first_partition); memcpy(out + first_partition, (void *)(data_ptr),
//     sizeof(float) * rb->index);
// }
...
inline void write_buffer_all(ring_buffer *rb, float (*data)[N_SAMPLES]) {
  for (int i = 0; i < N_SENSORS; i++) {
    for (int k = 0; k < N_SAMPLES; k += 8 * 2) {
      _mm256_storeu_ps(&rb->data[i][rb->index + k],
                       _mm256_loadu_ps(&data[i][k]));
      _mm256_storeu_ps(&rb->data[i][rb->index + (k + 8)],
                       _mm256_loadu_ps(&data[i][k + 8]));
    }
  }
...
void write_buffer_all(ring_buffer *rb, float (*data)[N_SAMPLES]) {
  for (int i = 0; i < N_SENSORS; i++) {
    memcpy((void *)&rb->data[i][rb->index], &data[i][0],
...
void read_buffer_all(ring_buffer *rb, float (*out)[BUFFER_LENGTH]) {
  ...
  for (int i = 0; i < N_SENSORS; i++) {
    memcpy(&out[i][0], (void *)&rb->data[i][rb->index],
           sizeof(float) * first_partition);
    memcpy(&out[i][0] + first_partition, (void *)&rb->data[i][0],
           sizeof(float) * rb->index);
  }
}
